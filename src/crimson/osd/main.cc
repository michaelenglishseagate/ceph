    ...
    return app.run(seastar_n_early_args.size(), const_cast<char**>(seastar_n_early_args.data()),
	  ...
	  if (config.count("trace")) {
	    seastar::global_logger_registry().set_all_loggers_level(
          ...
          if (const auto ret = logfile_set_ostream(local_conf()->log_file);
	      ret == -ENOENT) {
	    std::cerr << "log_file not found; continuing with default output"
	              << std::endl;
	  } else if (ret < 0) {
            std::cerr << "logfile_set_ostream failed with " << ret
